...
import {
  PropertyVerificationVerdictDto,
  PropertyVerificationVerdict,
} from '../../model/request/property-verification-verdict.dto';
import {
...
import { PropertyVersionService } from '../property/version/property-version.service';
...
export class PropertyVerificationService {
  ...
  async initiateVerification(
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (dto.propertyId) {
        ...
        if (activeVerification) {
          throw new BadRequestException(
            'An active verification request already exists for this property',
          );
        }
      } else {
      ...
      const verification = manager.create(PropertyVerification, {
        ...
        stageHistory: [
          { stage: VerificationStageStatus.INITIATED, completedAt: new Date() },
        ],
      });
  ...
  async updateVerification(
    ...
    userId: string,
    dto: UpdatePropertyVerificationDto,
  ): Promise<PropertyVerificationDto> {
    return await this.dataSource.transaction(async (manager) => {
      ...
      if (verification.stage !== VerificationStageStatus.INITIATED) {
        throw new BadRequestException(
          'Verification request can only be updated while in INITIATED stage',
        );
      }
      ...
      if (dto.propertyId) {
        if (verification.property.id !== dto.propertyId) {
          ...
          const activeVerification = await manager.findOne(
            PropertyVerification,
            {
              where: {
                property: { id: dto.propertyId },
          ...
          if (activeVerification) {
            throw new BadRequestException(
              'An active verification request already exists for this new property',
            );
          }
        ...
        if (
          prop.propertyVerificationStatus ===
          ...
          if (prop.location) {
            ...
            if (dto.address !== undefined) {
              prop.location.address = dto.address;
              locationUpdated = true;
            }
            if (dto.city !== undefined) {
              prop.location.city = dto.city;
              locationUpdated = true;
            }
            if (dto.state !== undefined) {
              prop.location.state = dto.state;
              locationUpdated = true;
            }
            if (dto.country !== undefined) {
              prop.location.country = dto.country;
              locationUpdated = true;
            }
            if (dto.polygon !== undefined) {
              prop.location.locationPolygon = dto.polygon;
              locationUpdated = true;
            }
  ...
  async submitVerification(
    ...
    if (verification.stage !== VerificationStageStatus.INITIATED) {
      throw new BadRequestException(
        'Verification request must be in INITIATED stage to be submitted',
      );
    }

    verification.stage = VerificationStageStatus.PENDING_ACCEPTANCE;
    this.appendStageHistory(
      verification,
      VerificationStageStatus.PENDING_ACCEPTANCE,
    );
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await manager.save(Property, verification.property);
      await manager.save(PropertyVerification, verification);
    });

    const savedVerification = await this.propertyVerificationRepository.findOne(
      {
  ...
  ): Promise<PaginationAndSortingResult<PropertyVerificationDto>> {
    const findOptions =
      PaginationAndSorting.createFindOptions<PropertyVerification>(
        null,
        queryDto,
        { user: { id: userId } } as FindOptionsWhere<PropertyVerification>,
        {},
        [
          'property',
          'property.location',
  ...
  async assignReview(
    ...
    if (verification.stage !== VerificationStageStatus.PENDING_ACCEPTANCE) {
      throw new BadRequestException(
        'Verification request must be in PENDING_ACCEPTANCE stage to be assigned',
      );
    }

    if (verification.reviewUser) {
      throw new BadRequestException(
        'Verification request is already assigned to an admin',
      );
    }
  ...
  async assignVerdict(
    verificationId: string,
    adminUserId: string,
    dto: PropertyVerificationVerdictDto,
  ): Promise<PropertyVerificationDto> {
    const verification = await this.propertyVerificationRepository.findOne({
      where: { id: verificationId },
    ...
    if (
      !verification.reviewUser ||
      verification.reviewUser.id !== adminUserId
    ) {
      throw new UnauthorizedException(
        'You are not the assigned reviewing admin for this request',
      );
    }
    ...
    if (dto.verdict === PropertyVerificationVerdict.ACCEPTED) {
      ...
      this.appendStageHistory(
        verification,
        VerificationStageStatus.VERIFICATION_REJECTED,
      );
      if (dto.comments) verification.adminComments = dto.comments;
  ...
  async adminAdvanceStage(
    ...
    > = {
      ...
      [VerificationStageStatus.STAGE_3]:
        VerificationStageStatus.VERIFICATION_COMPLETE,
    ...
    if (
      nextStage === verification.stage &&
      verification.stage !== VerificationStageStatus.VERIFICATION_COMPLETE
    ) {
      throw new BadRequestException(
        `Cannot artificially advance stage from ${verification.stage}`,
      );
    }
    ...
    if (
      savedVerification.stage === VerificationStageStatus.VERIFICATION_COMPLETE
    ) {
  ...
  async getAdminVerifications(
    ...
    const where: FindOptionsWhere<PropertyVerification> = {};
    if (stage) where.stage = stage;
    if (propertyId) where.property = { id: propertyId };
    if (userId) where.user = { id: userId };

    let customQuery = this.propertyVerificationRepository
    ...
    if (search) {
      customQuery = customQuery.andWhere('pv.caseId ILIKE :search', {
        search: `%${search}%`,
      });
    }
  ...
  private async getVerificationOrThrow(
    ...
    const verification = await this.propertyVerificationRepository.findOne({
      ...
      relations: [
        ...
        'property.parentProperty.location',
        'user',
        'reviewUser',
        'verificationFiles',
        'adminStageFiles',
  ...
  public convertToDto(
    ...
    return {
      ...
      user: verification.user
        ...
        : null,
      reviewUser: verification.reviewUser
        ? this.userService.convertToDto(verification.reviewUser)
        : null,
      stageHistory: verification.stageHistory || [],
