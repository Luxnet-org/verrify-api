...
export class AdminVerificationController {
  ...
  async getAdminVerifications(
    ...
    const result = await this.propertyVerificationService.getAdminVerifications(
      queryDto,
      stage,
      propertyId,
      userId,
  ...
  @Patch(':id/assign')
  @ApiOperation({
    summary: 'Assign a verification request to yourself (Admin)',
  })
  @SwaggerApiResponseData({ type: 'object', status: HttpStatus.OK })
  @HttpCode(HttpStatus.OK)
  async assignReview(
    ...
    const verification = await this.propertyVerificationService.assignReview(
      verificationId,
      user.userId,
    );
  ...
  @Patch(':id/verdict')
  @ApiOperation({
    summary: 'Assign verdict to a submitted verification request (Admin)',
  })
  @SwaggerApiResponseData({ type: 'object', status: HttpStatus.OK })
  @HttpCode(HttpStatus.OK)
  async assignVerdict(
    ...
    const user: UserInfo = request.user!;
    const verification = await this.propertyVerificationService.assignVerdict(
      verificationId,
      user.userId,
      body,
    );
  ...
  async advanceStage(
    ...
    const user: UserInfo = request.user!;
    const verification =
      await this.propertyVerificationService.adminAdvanceStage(
        verificationId,
        user.userId,
        payload.verificationFileIds,
