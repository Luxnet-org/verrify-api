...
import { AdminPropertyDetailsDto } from '../model/dto/property.dto';
...
export class AdminPropertyController {
  ...
  async overrideProperty(
    ...
    const property = await this.propertyService.adminOverrideProperty(
      propertyId,
      user.userId,
      payload,
    );
  ...
  > {
    const response: PaginationAndSortingResult<PropertyLookupResponseDto> =
      await this.propertyGetService.getAllProperties(searchQuery);
  ...
  @ApiOperation({
    summary:
      'Get current property details and active pending/reverification version',
  })
  @SwaggerApiResponseData({
    dataClass: AdminPropertyDetailsDto,
    status: HttpStatus.OK,
  })
  @Get('/:identifier')
  @HttpCode(HttpStatus.OK)
  async getOne(
    @Param('identifier') identifier: string,
  ): Promise<ApiResponse<AdminPropertyDetailsDto>> {
    const response = await this.propertyGetService.getOneForAdmin(identifier);
